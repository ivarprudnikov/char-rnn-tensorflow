...
class CharRNN:
        ...
        with tf.compat.v1.name_scope('inputs'):
            ...
            if self.use_embedding is False:
                ...
                with tf.device("/cpu:0"):
                    ...
                    self.lstm_inputs = tf.nn.embedding_lookup(params=embedding, ids=self.inputs)
    ...
    def build_lstm(self):
        ...
        with tf.compat.v1.name_scope('lstm'):
            ...
            with tf.compat.v1.variable_scope('softmax'):
                softmax_w = tf.Variable(tf.random.truncated_normal([self.lstm_size, self.num_classes], stddev=0.1))
    ...
    def build_loss(self):
        with tf.compat.v1.name_scope('loss'):
            ...
            loss = tf.nn.softmax_cross_entropy_with_logits(logits=self.logits, labels=tf.stop_gradient(y_reshaped))
            self.loss = tf.reduce_mean(input_tensor=loss)

    def build_optimizer(self):
        tvars = tf.compat.v1.trainable_variables()
        grads, _ = tf.clip_by_global_norm(tf.gradients(ys=self.loss, xs=tvars), self.grad_clip)
